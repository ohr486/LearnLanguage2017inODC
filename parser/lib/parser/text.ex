defmodule Parser.Text do
  ...
  defp char_impl(%State{status: :ok, line: line, column: col, input: <<next::utf8,_::binary>>} = state, c) do
    IO.puts "--- char_impl ---"
    IO.puts "line:#{inspect line}, col:#{inspect col}, c:#{inspect c}, state:#{inspect state}"
    IO.inspect is_binary(c)
    case c do
      c when is_binary(c) ->
